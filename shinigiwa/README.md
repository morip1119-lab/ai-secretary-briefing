...
```jsonc
{
  ...
  "hook": "1行目。人物名は出さず落差だけで惹きつける。数字は必ず『何の数字か』が分かる形で",
  "lead": "導入の2文。1文目で固有名詞、2文目までに必ずフルネームを出す",
  "story": {
    "sections": [
...
**ここの書き込みの粒度が、そのまま出力の粒度になる。** LLM は与えられた事実より具体的には書けない
（書いたらそれは捏造）。「依存に苦しんだ」としか書いていない台帳からは、「依存に苦しんだ」としか出てこない。

```jsonc
{
  "id": "…", "name": "…", "birthYear": 1969, "deathYear": 2023, "ageAtDeath": 54,
  "facts": [
    "最終盤のシーズンでは、主要キャスト6人がそれぞれ1話あたり100万ドル（およそ1億円）を受け取っていた",
    "シーズン3の終わりには、鎮痛剤バイコディンを1日55錠手に入れることに一日の大半を使っていた",
    "薬を切らさないために、売り出し中の家の内見に行っては備え付けの薬棚から抜いていた",
    "10年間で体重は58キロから102キロの間を行き来した",
    "2018年、49歳のときに大腸が破裂し、7時間の緊急手術を受けた",
    "家族は「今夜を越えられる可能性は2パーセント」と告げられた",
    "9か月つけていた人工肛門の袋は何度も破れ、その後も腹部の手術を14回受けた"
  ],
  "sources": ["…"]
}
```

悪い例（これを書くと、これしか出てこない）:
...
`data/subjects.json` の `matthew-perry` が、目指すべき書き込み量の見本。

**LLM は平気で年号と数字を間違える。** そのため guard に数値の突き合わせを入れてあり、
本文の年号が生年より前だったり未来だったり、享年が台帳と食い違うと警告が出る
（没後に起きた裁判などの年号は咎めない）。ただし**体重や金額の誤りは誰も止めてくれない**ので、
台帳に数字を書く時点で一次情報に当たること。
...
| 冒頭の名指し | エラー | フックとリードに人物名が無い原稿を弾く（誰の話か分からないまま読まれない） |
| 没後年数 | 警告 | 没後 7 年以内は「裏取りしろ」と警告（ブロックはしない） |
| 数値の整合 | 警告 | 本文の年号・享年が台帳と食い違うと警告（LLM の捏造対策） |
| 抽象語 | 警告 | 「壊れていた」「苦しんでいた」など、状態をまとめただけの語を検出 |
| 具体性 | 警告 | 数字の入った箇条書きが半分未満だと警告 |
